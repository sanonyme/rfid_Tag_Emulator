...
export function getAleCredentials(): { username: string; password: string } | null {
  const username = (process.env.ZEUS_ALE_USERNAME ?? process.env.VITE_ALE_USERNAME ?? '').trim()
  const password = process.env.ZEUS_ALE_PASSWORD ?? process.env.VITE_ALE_PASSWORD ?? ''
